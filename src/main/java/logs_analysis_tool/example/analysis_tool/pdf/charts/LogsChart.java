...
@Component
public class LogsChart extends XYChartImp {
    ...
    @Autowired
    public LogsChart(LogsRepo logsRepo) {
    ...
    public File chartBuilder() throws IOException {
        ...
        // Create Chart
        Chart chart = buildChart("Logging Chart", "DAY", "Logs Per Day");

        // Customize Chart
        customizeChart(chart);

        ((XYChart)chart).addSeries(" ", xData, yData);
        File tempFile = new File("loggingChart.png");
    ...
    {
        List<Long> yAxis = logsRepo.logInPerMonth(month);
